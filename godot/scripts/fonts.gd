...
#
# @tool: автозагрузка без @tool в редакторе Godot подменяется заглушкой без
# настоящих полей — main_menu.tscn (живой предпросмотр в редакторе) тогда не
# мог бы получить regular/bold. Здесь это безопасно: _ready() только строит
# ресурсы шрифтов, ни сохранений, ни окна, ни сети не трогает.
# ============================================================================
@tool
extends Node
