...
#
# @tool: без этой аннотации _draw() не вызывается для узлов, живущих внутри
# процесса редактора — а этот же класс переиспользует вкладка «Прогрессия
# перков» (godot/addons/perk_progression_editor). Скрипт трогает только
# статику PerkIcons, автозагрузок не касается — для игры ничего не меняется.
# ============================================================================
@tool
...
## Размер значка — раньше выставлялся только извне (custom_minimum_size
## после .new()) и нигде не был виден как настраиваемое свойство. Экспорт
## делает его редактируемым в инспекторе для карточек-сцен (upgrade_card.
## tscn и т.д.); старые вызовы, которые сами ставят custom_minimum_size
## после .new(), это не задевает — их присваивание просто идёт следом.
