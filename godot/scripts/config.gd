...
#
# @tool: без этого статические var (UI_TEXT и т.п.) в редакторе Godot не
# инициализируются вовсе (пустой Color(0,0,0,1) вместо заданного) — эта же
# болячка нашлась и чинится тем же способом у PerkIcons/Upgrades/Cosmetics/
# Achievements/Perks/MenuTips/UiKit, см. их шапки. Чистые данные, побочных
# эффектов нет — безопасно.
# ============================================================================
@tool
class_name Cfg
extends RefCounted
...
## Потолок на длину повтора команд за одну сверку клиентского предсказания
## (см. game.gd::_reconcile_local_tank) — 2 сек при 60 Гц, чтобы аномально
## большой разрыв (реконнект/подвисание) не устроил спайк за один кадр.
...
## Доступные игроку расцветки танка (ключи TEAM_COLORS). level — уровень
## профиля (Prof.global_level), с которого расцветка открыта в Гараже;
## p1/p2 открыты сразу, остальные совпадают с порогами Ranks.LIST —
## новый цвет достаётся вместе со званием.
const PLAYER_SKINS := [
...
# --------------------------------------------------------------- гроза
## Урон от прямого попадания молнии. Ровно столько и ни сколько иначе:
## это не «взрыв рядом», а удар в танк.
const LIGHTNING_DAMAGE := 90.0
## Радиус поражения вокруг точки удара.
const LIGHTNING_RADIUS := 34.0
## Сколько тиков между попытками ударить и с какой вероятностью попытка
## удаётся. Раньше гроза мигала всем экраном по два раза в секунду —
## теперь бьёт редко, но по-настоящему.
const LIGHTNING_EVERY := 90
const LIGHTNING_CHANCE := 0.55
## «Повелитель молний»: шанс каждого носителя перка ударить молнией в
## ближайшего врага в тот же ритм, что и обычный разряд.
const LIGHTNING_LORD_CHANCE := 0.25
## С «Небесным ударом» и «Цепной молнией» одновременно (полный грозовой
## билд) шанс вырастает втрое — иначе третий слот перков тратился бы без
## заметной отдачи именно на этот бросок.
const LIGHTNING_LORD_SYNERGY_CHANCE := 0.75

## «Небесный удар»: кулдаун заряда следующего выстрела, тиков (12 секунд).
## Копится только в грозу — как и сам «Повелитель молний».
const SKY_STRIKE_COOLDOWN := 720

## «Цепная молния»: удар молнии от танка с этим перком дополнительно бьёт
## по ближайшим врагам в этом радиусе (шире обычного радиуса поражения
## LIGHTNING_RADIUS — иначе почти всегда совпадал бы с ним и цеплять было
## бы некого).
...
## Замер до правки: игрок делал 3.16 выстрела в секунду против 1.30
## у среднего бота — быстрее в два с половиной раза, а с перком
## «Скорострельность» вчетверо. Отсюда 27 тиков вместо 19 и перегрев ствола.
const PLAYER_FIRE_RATE := 27

# ------------------------------------------------------------- перегрев ствола
## Нагрев за выстрел и остывание за тик подобраны замером непрерывной
## стрельбы, а не на глаз. Первая пара (0.16 и 0.0075) не работала вовсе:
## при 27 тиках между выстрелами остывание за интервал (0.20) обгоняло
## нагрев (0.16), и перегрев не наступал никогда — счётчик показал ноль
## срывов за партию.
##
## Итог по замеру устойчивого темпа непрерывной стрельбы:
##   было (19 тиков, без перегрева) — 3.17 выстрела/с
##   стало                          — 1.32 выстрела/с
##   бот на средней сложности       — 1.30 выстрела/с
##
## С перком «Скорострельность» темп остаётся 1.28/с: перк теперь покупает
## очередь, а не бесконечный поток.
## Урон игрока за выстрел. Резать один темп было нельзя: замер показал,
## что «Оборона» после этого падала на 4-6 волне вместо 6-7 — вклад игрока
## в удержание базы слишком велик. Поэтому выстрелов стало меньше, но
## каждый весомее: суммарный урон в секунду упал с 79 до 52 (у бота 33),
## и стрельба перестала быть потоком, не превратившись в беспомощность.
const PLAYER_DMG_MULT := 1.5

## «Полевой ремонт» — доля запаса прочности за нажатие.
const REPAIR_FRACTION := 0.33
## «Кумулятив» — множитель урона по постройкам, пока способность активна.
const BREAKER_BUILDING_MULT := 4.0
## Дальность, с которой бот слышит выстрел и идёт проверять.
const BOT_HEAR_RANGE := 520.0
## «Дымовая завеса» — дальше этого бот вас не видит, пока она держится.
const SMOKE_VISION := 150.0

const HEAT_PER_SHOT := 0.20
const HEAT_COOL := 0.0040
## Перегрев наступает на единице, а стрелять снова можно только когда жар
## упадёт до этой отметки. Порог возврата ниже порога срыва намеренно:
## без этого зазора танк на пределе стрелял бы рывками по одному выстрелу.
const HEAT_RESUME := 0.30

## «Глушение»/«Хищник»: сколько тиков после последнего попадания именно ОТ
## ЭТОГО атакующего цель ещё считается «не заметившей» его для бонуса
## внезапной атаки. Общий для игроков и ботов — раньше бонус проверял только
## текущую цель бота (target.brain.target != attacker), что не работало
## против игроков вовсе (у них нет brain). last_attacker/last_attacker_tick
...
## этой дистанции (те же 50 м, что и у испытания самого «Снайпера», см.
## world.gd:_credit_player_kill) само по себе критическое.
...
## Гарантированный таран по замороженному БОССУ (execute_frozen_kill) не
## убивает сразу, а снимает эту долю максимума HP — полноценная встреча с
## боссом не должна сводиться к одному тарану даже с «Ледяной пушкой».
## Обычных врагов по-прежнему убивает мгновенно.
const FROZEN_BOSS_RAM_FRACTION := 0.5

## «Кислотная пушка» — стакающийся яд.
const ACID_STACK_MAX := 5
const ACID_DURATION_TICKS := 300     # 5 секунд, обновляется каждым попаданием
const ACID_TICK_INTERVAL := 30       # тик урона раз в 0.5 с
const ACID_DMG_PER_STACK_TICK := 6.0 # урон одного стака за один тик

## «Едкое облако»: на максимуме стаков цель раз в тик забрызгивает ближайших
## врагов (относительно того, кто наложил яд) собственным стаком.
const ACID_CLOUD_RADIUS := 80.0
## «Разъедающая броня»: множитель урона от НЕ-кислотных источников, пока на
## цели есть хоть один стак, наложенный носителем этого перка.
const CORRODING_ARMOR_MULT := 1.4
## Билд «Кислотный охотник» (corrosive_acid+acid_cloud+corroding_armor)
## открывает активку «Кислотная бомба»: как «Ударная волна», только вместо
## урона — стаки яда всем врагам в радиусе Cfg.SHOCKWAVE_R.
const ACID_BOMB_STACKS := 3
const ACID_BOMB_COOLDOWN := 900      # 15 секунд
