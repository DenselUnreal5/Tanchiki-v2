...
var EN := {
	...
	"perk.quick_reload.desc": "Reload 20% faster, but the barrel heats up half as much — sustains fire longer than Rapid Fire",
	...
	"perk.lightning_lord.desc": "During a storm — 25% chance lightning strikes the nearest enemy tank",
	"perk.ram.name": "Ram",
	"perk.ram.desc": "Double damage on collision — ram with confidence. Combined with Thick Armor and Kamikaze, a ram kill sends out a small shockwave at nearby enemies",
	...
	"perk.sniper.desc": "Shells fly 25% faster and hit 10% harder. Combined with Evasion and Keen Ear, a long-range shot becomes critical",
	...
	"perk.turbo.desc": "Every kill grants 4 seconds of ×1.5 speed",
	"perk.turbo.challenge": "Kill 5 enemies in 10 seconds",
	"perk.shadow.name": "Shadow",
	"perk.shadow.desc": "After destroying an enemy tank you become fully invisible to bots and enemies — on the minimap and on screen — for 5 seconds. The effect stacks with every new kill",
	...
	"perk.keen_ear.desc": "Upgraded mics pick up distant shots and mark them on the minimap, and also reveal nearby enemies hidden by Shadow",
	"perk.muffler.name": "Muffler",
	"perk.muffler.desc": "Your shots are heard from half the distance, but damage against an enemy who hasn't spotted you is up 50%. Also makes you harder for Keen Ear to detect while cloaked by Shadow",
	"perk.predator.name": "Predator",
	"perk.predator.desc": "Every ambush hit grants a 2-second speed dash — strike and vanish. Combined with Forest Dweller and Shadow, turns an ambush into a critical hit",
	"perk.sky_strike.name": "Sky Strike",
	"perk.sky_strike.desc": "During storms, every 12 seconds your next shot also calls down lightning at the point of impact on top of its normal damage (90 damage, scorched trail)",
	"perk.chain_lightning.name": "Chain Lightning",
	"perk.chain_lightning.desc": "Your lightning strikes arc to the 3 nearest enemy tanks in range, dealing them 50% damage",
	"perk.deep_freeze.name": "Deep Freeze",
	"perk.deep_freeze.desc": "The Ice Cannon's freeze lasts 50% longer. Combined with Frost Dash and Chilled Barrel, the shot itself finishes off a frozen target without a ram (except bosses)",
	"perk.frost_dash.name": "Frost Dash",
	"perk.frost_dash.desc": "Landing a freeze grants 1.5 seconds of speed dash — enough to close in and ram before the target thaws",
	"perk.chilled_barrel.name": "Chilled Barrel",
	"perk.chilled_barrel.desc": "The Ice Cannon's triple heat penalty is cut by 35% — fire more before overheating after the first shot",
	"perk.corrosive_acid.name": "Corrosive Acid",
	"perk.corrosive_acid.desc": "Every tick of the stacking poison hits 50% harder",
	"perk.acid_cloud.name": "Acid Cloud",
	"perk.acid_cloud.desc": "A target at max stacks (5) occasionally splashes nearby enemies with a stack of its own",
	"perk.corroding_armor.name": "Corroding Armor",
	"perk.corroding_armor.desc": "While a target carries acid stacks, it takes 40% more damage from any other source — the acid eats through armor",
	...
	"perk.silencer.desc": "6 seconds where bots can't hear your shots at all, and any hit you land counts as a surprise attack",
	...
	"build.lightning.bonus": "All three perks together: the storm is summoned and never lets up for the rest of the match, and Lightning Lord's chance to strike triples from 25% to 75%",
	"build.stealth_hunter.name": "Ambush build",
	"build.stealth_hunter.bonus": "All three perks together: an ambush hit becomes critical, dealing 2.5x damage",
	"build.juggernaut.name": "Juggernaut build",
	"build.juggernaut.bonus": "All three perks together: a ram kill sends out a small shockwave — pushes and clips enemy tanks near the collision",
	"build.ghost_sniper.name": "Ghost Sniper build",
	"build.ghost_sniper.bonus": "All three perks together: a hit from 50m or beyond becomes critical, dealing 1.6x damage",
	"build.ice_hunter.name": "Ice Hunter build",
	"build.ice_hunter.bonus": "All three perks together: an Ice Cannon hit finishes off the frozen target by itself — no ram needed (except bosses)",
	"build.acid_hunter.name": "Acid Hunter build",
	"build.acid_hunter.bonus": "All three perks together: unlocks the Acid Bomb active ability — like Shockwave, but instead of damage it gives every enemy in the blast radius 3 stacks of poison (15s cooldown)",
