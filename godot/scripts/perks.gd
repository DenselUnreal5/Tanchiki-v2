...
#
# @tool: без этого статические списки ненадёжны в редакторе Godot — см. Cfg
# для полного объяснения. Чистые данные, безопасно.
# ============================================================================
@tool
class_name Perks
extends RefCounted
...
const LIST := [
	...
	},
	{
		...
		"desc": "Перезарядка быстрее на 20%, зато ствол греется вдвое меньше — держит непрерывный огонь дольше «Скорострельности»", "category": "fire",
		# Раньше уступал «Скорострельности» по обеим осям разом (0.7/0.8
		# против 0.6/0.7) — чистая ловушка выбора с момента, когда открыты
		# оба перка. Теперь разные роли: «Скорострельность» — взрывной темп
		# ценой быстрого перегрева, «Быстрая перезарядка» — скромнее в темпе,
		# но держит очередь заметно дольше не перегреваясь.
	...
	},
	{
		...
		"desc": "Во время грозы — 25% шанс, что молния ударит по ближайшему вражескому танку",
		"category": "special", "flags": ["lightningLord"],
	},
	{
		"id": "sky_strike", "name": "Небесный удар", "icon": "⚡",
		"desc": "В грозу раз в 12 секунд следующий выстрел вдобавок к обычному урону бьёт молнией в точке попадания (90 урона, обугленный след)",
	...
	# ------------------------------------------------------- челленджи
	{
		...
		"desc": "Удвоенный урон при столкновении — тараньте смело. Вместе с «Толстой бронёй» и «Камикадзе» превращает убийство тараном в маленькую ударную волну по соседям",
		"category": "challenge",
		"flags": ["ram"],
		"mods": {"ramMult": 2.0},
	...
	},
	{
		...
		"desc": "Каждое убийство даёт 4 секунды ускорения ×1.5", "category": "challenge",
		# Испытание («5 фрагов за 10 секунд») — одно из самых требовательных
		# в игре, а награда была тоньше, чем у сопоставимых испытаний
		# («Снайпер»/«Берсерк» дают боевую прибавку). 3→4 секунды.
		"mods": {"turboOnKill": 240.0},
		"challenge": {"desc": "Убей 5 врагов за 10 секунд", "stat": "rapidKills", "need": 5},
	},
	{
		"id": "shadow", "name": "Тень", "icon": "🌑",
		"desc": "После уничтожения врага вы на 5 секунд полностью пропадаете из виду ботов и врагов — с миникарты и с экрана. Эффект суммируется с каждым новым убийством", "category": "challenge",
		"flags": ["shadow"],
...
const EXTRA_LIST := [
	...
	},
	{
		...
		"desc": "Тихий выстрел слышен вдвое ближе, зато урон по врагу, который вас не заметил, выше на 50%. Заодно усложняет обнаружение «Острым слухом», если вы замаскированы «Тенью»",
		"category": "special",
	...
	},
	{
		...
		"desc": "Каждое попадание из засады даёт 2 секунды рывка скорости — ударил и ушёл. Вместе с «Лесным жителем» и «Тенью» превращает засаду в критический удар",
		"category": "special",
		"flags": ["predator"],
		# Срабатывает от того же события, что и бонус «Глушения» (попадание
		# по цели, которая вас не засекла), но не требует его: без «Глушения»
		# это просто рывок после внезапного попадания, с ним — засадный урон
		# и отрыв одним комбо. Общий турбо-таймер с перком «Турбо» — тот же
		# ресурс, просто ещё один источник его завести.
...
const EXTRA_ACTIVE := [
	...
	},
	{
		...
		"desc": "6 секунд ваши выстрелы совсем не слышны ботам, и любое попадание в этот момент считается внезапной атакой", "category": "special",
		"active": "silencer",
	},
	{
...
const UNLOCK_TABLE := {
	...
	# «Хищник» добавлен третьим сюда же, а не отдельным уровнем: он читается
	# как продолжение «Глушения» на том же уровне, а не отдельная ступень
...
const BUILDS := [
	{"id": "lightning", "name": "Грозовой билд",
		...
		"bonus": "Все три перка сразу: гроза призывается и не утихает до конца партии, а шанс «Повелителя молний» ударить молнией растёт с 25% до 75%"},
	{"id": "stealth_hunter", "name": "Засадный билд",
		"perks": ["predator", "forest", "shadow"],
		"bonus": "Все три перка сразу: попадание из засады становится критическим и наносит в 2,5 раза больше урона"},
