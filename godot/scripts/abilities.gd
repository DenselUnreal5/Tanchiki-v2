...
static var LIST := [
	...
	},
	{
		...
		"desc": "6 с выстрелы не слышны ботам, попадания — внезапная атака",
	...
	# Не выдаётся никаким отдельным перком — открывается только билдом
	# «Кислотный охотник» (см. Perks.BUILDS, Perks.active_ability_of()),
	# собранным из трёх пассивных перков «Кислотной пушки».
	{
		# Мгновенная, как «Ударная волна» — тот же радиус (Cfg.SHOCKWAVE_R),
		# но вместо урона и построек — стаки яда по танкам.
		"id": "acid_bomb", "name": "Кислотная бомба", "icon": "🧪",
		"desc": "Взрыв вокруг: все враги в радиусе получают 3 стака яда",
		"color": Color("#9dff5c"), "cooldown": Cfg.ACID_BOMB_COOLDOWN, "duration": 0,
	},
